...
class Summary:
    ...
    def add_data(self, profile: Profile, Tweets_result: list):
        with self._lock:
            if profile.get_username() not in self.profiles_summary:
                self.profiles_summary[profile.get_username()] = {
                    "added_tweet_count": len(Tweets_result),
                }
            else:
                self.profiles_summary[profile.get_username()]["added_tweet_count"] += len(Tweets_result)
