...
class TestPearsonCorrelationProfileStatGenerator(TestCase):
    ...
    def test_01_pearson(self):
        ...
        tweets = [
            {
                "tweet": self.helper.ReturnP1Tweet1(),
        ...
        calculated_metrics = {
            "pearson-tweet_likes-vs-tweet_retweets": np.corrcoef(
                likes, [x["metrics"][0] for x in tweets]
            )[0, 1],
            "pearson-tweet_likes-vs-tweet_replies": np.corrcoef(
                likes, [x["metrics"][1] for x in tweets]
            )[0, 1],
            "pearson-tweet_likes-vs-tweet_quotes": np.corrcoef(
                likes, [x["metrics"][3] for x in tweets]
            )[0, 1],
            "pearson-tweet_likes-vs-tweet_chars": np.corrcoef(
                likes, [len(x["tweet"].get_text()) for x in tweets]
            )[0, 1],
            "pearson-tweet_likes-vs-tweet_words": np.corrcoef(
                likes, [len(x["tweet"].get_text().split()) for x in tweets]
            )[0, 1],
            "pearson-tweet_likes-vs-post_date_hour": np.corrcoef(
                likes, [x["tweet"].get_post_date().hour for x in tweets]
            )[0, 1],
            "pearson-tweet_likes-vs-post_date_day": np.corrcoef(
                likes, [x["tweet"].get_post_date().weekday() for x in tweets]
            )[0, 1],
        }
